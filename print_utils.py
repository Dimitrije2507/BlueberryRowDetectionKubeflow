...
def pretraining_prints(p_index,lr,stepovi,lambda_parametri,batch_size,loss_type):
    tmp = get_args('train')
    globals().update(tmp)
    ...
    logs_path = base_folder_path + "/logs/" + script_name + "/"+ str(p_index) + ": "+ today + " lr "+str(lr) + ", step na " + str(stepovi) +" epoha, " + "lambda parametar " + str(lambda_parametri) +" batch_size: "+str(batch_size)+ " sched " + str(scheduler_lr) + " loss " + loss_type  # path to the folder that we want to save the logs for Tensorboard
    if not os.path.exists(logs_path):
        os.mkdir(logs_path)
...
def after_data_loading_prints(lr,ime_foldera_za_upis,train_loader,valid_loader):
